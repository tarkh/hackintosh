...
* Если вы идете в хронологическом порядке по данной инструкции и еще не перепрошили BIOS для возможности разблокировки турбо-буста м андервольтинга, то убедитесь, что предустановленный efi-модуль `v3x2_80-50-50_39_vcc1.8.efi` деактивирован. Для этого в `EFI/OC/config.plist` в `UEFI > Drivers` находите запись с параметром `path`: `v3x2_80-50-50_39_vcc1.8.efi` и смотрите, чтобы значение поля `Enabled` было `false`.

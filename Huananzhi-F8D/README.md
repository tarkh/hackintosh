# Huananzhi F8D
**([русская версия](https://github.com/tarkh/hackintosh/edit/main/Huananzhi-F8D/README-rus.md))**
...
To run macOS, we need the `./EFI` directory, which must be copied entirely to the root of the USB drive, having previously formatted it in **fat32**.
...
* Also, if the processors are different, later it will not be superfluous to reset `CPUFriendDataProvider.kext`, because it can also be made for a specific processor. Detailed information [available here](https://dortania .github.io/OpenCore-Post-Install/universal/pm.html#using-cpu-friend).
